...
moment.fn.workingDiff = function workingDiff(comparator, unit, detail) {
    ...
    while(to.diff(from, "hours", true) > 24) {
        if (unit === 'day') {
            diff--;
            from = from.addWorkingTime(1, 'day');
        } else {
            diff += from.diff(openingTimes(from)[1], unit, true);
            console.log(diff, openingTimes(from)[1], from)
            from = openingTimes(from.nextWorkingDay())[0];
        }
    }
    if (unit !== 'day'){
        var time = openingTimes(from)[1]
        if(from.isBefore(time) && to.isAfter(time)){
          var one = time.diff(from, unit, true);
          time = openingTimes(to)[0]
          var two = to.diff(time, unit, true);
          diff -= one + two
        }else{
